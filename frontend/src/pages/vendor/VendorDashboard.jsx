...
import { produtoService, vendedorService } from '../../services/api';
...
function VendorDashboard() {
  ...
  // Usuário vendedor mockado - em produção viriam do contexto/estado global
  const vendorUser = {
    nome: "Vendedor Exemplo",
    email: "vendedor@helpnet.com",
    id: "VEN-0001"
  };
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      try {
        // Para vendedor, filtrar produtos por empresa/vendedor
        const [all, active, noStock] = await Promise.all([
          produtoService.listarVendedor(1, { limit: 1 }), // empresaId mock
          produtoService.listarVendedor(1, { status: 'ativo', limit: 1 }),
          produtoService.listarVendedor(1, { status: 'sem-estoque', limit: 1 }),
        ]);
        if (!isMounted) return;
  ...
  useEffect(() => {
    const loadDashboard = async () => {
      try {
        const data = await vendedorService.dashboard();
  ...
  // KPIs do painel do vendedor
  const kpis = [
    { titulo: "Receita", valor: dashboardData ? `R$ ${dashboardData.receita.toFixed(2)}` : "R$ 0.00", icone: <FiBarChart2 />, cor: "bg-green-600" },
    { titulo: "Pedidos", valor: dashboardData ? String(dashboardData.pedidos) : "0", icone: <FiPackage />, cor: "bg-green-500" },
    { titulo: "Clientes", valor: dashboardData ? String(dashboardData.clientes) : "0", icone: <FiUsers />, cor: "bg-green-400" },
    { titulo: "Produtos", valor: productsTotal !== null ? String(productsTotal) : "0", icone: <FiBox />, cor: "bg-green-700" },
  ];
  ...
  const menuVendedor = [
    ...
    { label: 'Meus Produtos', to: '/vendedor/produtos', icon: <FiBox className="text-slate-500" /> },
  ...
  return (
    <AdminLayout menuItems={menuVendedor}>
      <div className="px-4 sm:px-6 lg:px-8 py-8">
          {/* Boas-vindas / Introdução */}
          <section className="mb-8">
            <div className="bg-white border border-slate-200 rounded-xl p-6">
              <h2 className="text-xl font-semibold text-slate-900 mb-1">Bem-vindo ao Painel do Vendedor</h2>
              <p className="text-slate-600 text-sm">Gerencie seus produtos, pedidos, clientes e relatórios de vendas.</p>
            </div>
          </section>
          ...
          <section className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            <div className="bg-white rounded-xl shadow-sm p-6 border border-slate-200">
              <div className="flex items-center">
                <div className="bg-green-700 p-3 rounded-lg text-white mr-4">
                  <FiBox />
                </div>
                <div>
              ...
              <div className="bg-white rounded-xl shadow-sm border border-slate-200">
                ...
                <div className="p-6">
                  ...
                  <div className="mt-6">
                    <Link
                      to="/vendedor/pedidos"
                      className="text-green-700 hover:text-green-800 font-medium text-sm hover:underline"
                    >
                ...
                <div className="p-6">
                  <div className="space-y-3">
                    <Link
                      ...
                      className="flex items-center space-x-3 p-3 rounded-lg hover:bg-green-50 transition-colors group border border-transparent hover:border-green-200"
                    >
                      <FiPlus className="text-slate-400 group-hover:text-green-700" />
                    ...
                    <Link
                      ...
                      className="flex items-center space-x-3 p-3 rounded-lg hover:bg-green-50 transition-colors group border border-transparent hover:border-green-200"
                    >
                      <FaUser className="text-slate-400 group-hover:text-green-700" />
                      <span className="text-slate-700 group-hover:text-slate-900">Ver Clientes</span>
                    </Link>
                    <Link
                      to="/vendedor/relatorios"
                      className="flex items-center space-x-3 p-3 rounded-lg hover:bg-green-50 transition-colors group border border-transparent hover:border-green-200"
                    >
                      <FiBarChart2 className="text-slate-400 group-hover:text-green-700" />
                      <span className="text-slate-700 group-hover:text-slate-900">Relatórios</span>
                    </Link>
                  </div>
                </div>
              </div>
                ...
                <div className="p-6">
                  <div className="space-y-3">
                    <div>
                      ...
                      <p className="font-medium text-slate-900">{vendorUser.nome}</p>
                    </div>
                    <div>
                      <p className="text-xs text-slate-500">E-mail</p>
                      <p className="font-medium text-slate-900">{vendorUser.email}</p>
                    </div>
                    <div>
                      <p className="text-xs text-slate-500">ID do Vendedor</p>
                      <p className="font-mono font-medium text-slate-900">{vendorUser.id}</p>
                    </div>
                  </div>
                </div>
              </div>
