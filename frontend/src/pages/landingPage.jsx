...
import { FaShoppingCart, FaUser, FaBuilding, FaShieldAlt, FaTruck, FaHeadset, FaStar, FaCreditCard, FaGlobe, FaAward, FaPhone, FaEnvelope, FaMapMarkerAlt, FaStore, FaSpinner } from 'react-icons/fa';
    ...
    const fetchData = async () => {
      try {
        ...
        if (testimonialsResponse.success) {
          console.log('Avaliações recebidas:', testimonialsResponse.avaliacoes);
          // Mapear as avaliações para o formato esperado
          const depoimentosMapeados = testimonialsResponse.avaliacoes.map(avaliacao => ({
            nome: avaliacao.cliente.NomeCompleto,
            comentario: avaliacao.Comentario,
            estrelas: avaliacao.Nota,
            tipo: "Cliente Verificado"
          }));
          setDepoimentos(depoimentosMapeados);
        } else {
          console.log('Erro ao buscar avaliações:', testimonialsResponse);
        }
      } catch (error) {
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-16">
            ...
            <p className="text-xl text-slate-600">
              Depoimentos e avaliações reais de quem já usa nossa plataforma
            </p>
          </div>
            ...
            {loadingDepoimentos ? (
              ...
              depoimentos.length <= 3 ? (
                <div className="flex justify-center">
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-6xl">
                    {depoimentos.map((depoimento, index) => (
                      <div key={index} className="bg-white p-8 rounded-2xl shadow-xl hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-2 border-l-4 border-blue-500 relative min-h-[320px] flex flex-col">
                        ...
                        <p className="text-slate-700 mb-6 leading-relaxed text-lg flex-1">
                ...
                <div className="overflow-x-auto scrollbar-hide">
                  <div className="flex gap-8 pb-4 px-4 justify-center" style={{ width: 'max-content' }}>
                    {depoimentos.map((depoimento, index) => (
                      <div key={index} className="bg-white p-8 rounded-2xl shadow-xl hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-2 border-l-4 border-blue-500 relative w-96 flex-shrink-0 min-h-[320px] flex flex-col">
                        <div className="absolute top-4 right-4 text-blue-500 text-4xl opacity-20">
                          "
                        </div>
                        <div className="flex items-center mb-6">
              ...
              <div className="flex justify-center">
                <div className="bg-gradient-to-br from-blue-50 to-indigo-50 p-12 rounded-3xl shadow-2xl border border-blue-100 max-w-2xl text-center relative overflow-hidden">
                  ...
                  <div className="relative z-10">
                    ...
                    <p className="text-slate-600 text-lg leading-relaxed mb-6">
                      Compartilhe sua opinião sobre nosso sistema e-commerce.
                    </p>
                    <div className="text-sm text-slate-500">
                      Cadastre-se ou faça login para avaliar.
                    </div>
                  </div>
                </div>
              </div>
